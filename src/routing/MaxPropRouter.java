...
public class MaxPropRouter extends ActiveRouter {
		...
		for (Message m : messages) {
			if (excludeMsgBeingSent && isSending(m.getId())) {
				continue; // skip the message(s) that router is sending
				          // lewati pesan yang dikirimkan router
			}
			validMessages.add(m);
		}
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		// untuk semua host terhubung yang tidak mentransfer saat ini, 
		// kumpulkan semua pesan yang dapat dikirim
		for (Connection con : getConnections()) {
			...
			if (othRouter.isTransferring()) {
				continue; // skip hosts that are transferring
				          // lewati host yang mentransfer 
			}

			for (Message m : msgCollection) {
				/* skip messages that the other host has or that have
				   passed the other host */
				//melewatkan pesan yang dimiliki host lain atau yang 
				//telah melewati host lain
				...
				// lewati pesan jika host ini telah mengirimnya ke host lain 
				// (terlepas dari apakah host lain masih memilikinya)
				if (sentMsgIds != null && sentMsgIds.contains(m.getId())) {
					continue;
				}
				/* message was a good candidate for sending */
				// pesan ini adalah kandidat yang bagus untuk dikirim
		...
		// mengurutkan tupel message-connection sesuai dengan kriteria yang 
		// ditentukan dalam MaxPropTupleComparator
