 ...
 */
public class MaxPropRouterManajemenBuffer extends ActiveRouter {
	 ...
	 */
	public MaxPropRouterManajemenBuffer(Settings settings) {
		super(settings);
	 ...
	 */
	protected MaxPropRouterManajemenBuffer(MaxPropRouterManajemenBuffer r) {
		super(r);
		this.alpha = r.alpha;
	...
	public void changedConnection(Connection con) {
		...
		if (con.isUp()) { // new connection
			...
			this.costsForMessages = null; // invalidate old cost estimates
										 // membatalkan perkiraan cost lama
			if (con.isInitiator(getHost())) {
				...
				/*inisiator melakukan semua tindakan atas 
				 * nama node lain juga (sehingga probs pertemuan diperbarui 
				 * untuk keduanya sebelum bertukar)
				 */
				DTNHost otherHost = con.getOtherNode(getHost());
				MessageRouter mRouter = otherHost.getRouter();

				assert mRouter instanceof MaxPropRouterManajemenBuffer : "MaxProp only works "+
				" with other routers of same type";
				MaxPropRouterManajemenBuffer otherRouter = (MaxPropRouterManajemenBuffer)mRouter;

				/* exchange ACKed message data */
				// bertukar data pesan ACKed
				...
				// perbarui kedua peluang pertemuan
				...
				// menukar probabilitas transitif
			...
			// koneksi turun, memperbarui rata-rata byte yang ditransfer
	 ...
	 * Perbarui nilai probabilitas transitif dengan mengganti arus
	 * MeetingProbabilitySets with the values from the given mapping
	 * MeetingProbabilitySets dengan nilai-nilai dari pemetaan yang diberikan
	 * if the given sets have more recent updates.
	 * jika set yang diberikan memiliki pembaruan yang lebih baru.
	 ...
	 * Menghapus pesan dari buffer pesan yang dikenal sebagai ACKed
	 */
	...
	public Message messageTransferred(String id, DTNHost from) {
		this.costsForMessages = null; // new message -> invalidate costs
									  // pesan baru -> membatalkan cost
		Message m = super.messageTransferred(id, from);
		/* was this node the final recipient of the message? */
		// apakah node ini penerima pesan terakhir?
	 ...
	 * Method ini disebut tepat sebelum transfer diselesaikan
	 * at {@link ActiveRouter#update()}. MaxProp makes book keeping of the
	 * di {@link ActiveRouter#update()}. MaxProp membuat pembukuan
	 * delivered messages so their IDs are stored.
	 * pesan tersampaikan sehingga IDs mereka disimpan
	 * @param con The connection whose transfer was finalized
	 * @param con Koneksi yang transfernya diselesaikan
	 */
	@Override
	 ...
	 * Perbarui perkiraan rata-rata jumlah byte yang ditransfer per 
	   peluang transfer
	 * @param newValue The new value to add to the estimate
	 * @param newValue Nilai baru untuk ditambahkan ke perkiraan
	 */
	private void updateTransferredBytesAvg(int newValue) {
		...
		for (int i=0; i < BYTES_TRANSFERRED_AVG_SAMPLES; i++) {
			if (this.avgSamples[i] > 0) { // only values above zero count 
										  //hanya nilai di atas jumlah nol
				realCount++;
		...
		if (realCount > 0) {
			this.avgTransferredBytes = sum / realCount;
			   ...
			   // tidak ada sampel atau semua sampel nol
	 ...
	 * Returns the next message that should be dropped, according to MaxProp's
	   ...
	   Mengembalikan pesan berikutnya yang harus dibuang, sesuai dengan skema 
	   pemesanan pesan MaxProp (lihat MaxPropTupleComparator)
	 * @param excludeMsgBeingSent If true, excludes message(s) that are
	   ...
	   @param excludeMsgBeingSent Jika benar, kecualikan pesan yang 
	   dikirim dari pemeriksaan next-to-be-dropped (yaitu, jika pesan berikutnya 
	   dihapus sedang dikirim, pesan berikut ini dikembalikan)
	 * @return The oldest message or null if no message could be returned
	   @return pesan tertua atau null jika tidak ada pesan yang bisa dikembalikan
	   (no messages in buffer or all messages in buffer are being sent and
	   exludeMsgBeingSent is true)
	   (tidak ada pesan dalam buffer atau semua pesan dalam buffer sedang dikirim 
	   dan dikecualikan oleh pesan instan)
	 */
    @Override
	protected Message getNextMessageToRemove(boolean excludeMsgBeingSent) {
		Collection<Message> messages = this.getMessageCollection();
		//List<Message> validMessages = new ArrayList<Message>();
		System.out.print(String.format("%.2f", SimClock.getTime())+" Node: "+this.getHost()+" pesan : ");
		Message oldest = null;
		
		//looping semua pesan yg ada
		for (Message m : messages) {
			System.out.print(m.getId()+"("+m.getHopCount()+","+m.getTtl()+");");
			
			if (excludeMsgBeingSent && isSending(m.getId())) {
				System.out.print("sending("+m.getId()+");");
				continue; // skip the message(s) that router is sending
			}
			
			//start edit
			
			//set nilai oldest(kandidat pesan yg akan di drop)
			if (oldest == null ) {
				oldest = m;
			}
			
			//membandingkan hop count
			else if(oldest.getHopCount() == m.getHopCount()){
				//jika hop count sama maka dibandingkan TTL nya, mana yg lebih kecil
				if(m.getTtl() < oldest.getTtl()){
					oldest = m;
				}
			}
			
			//jika hop count oldest lebih kecil dari pesan yg sedang dibandingkan maka oldest = m(pesan saat ini)
			else if (oldest.getHopCount() < m.getHopCount()) {
				oldest = m;
			}
		}	
		System.out.print("dropped "+oldest.getId()+"\n");
		return oldest;
	}
	@Override
	public void update() {
		...
		if (!canStartTransfer() ||isTransferring()) {
			return; // nothing to transfer or is currently transferring
			        // tidak ada transfer atau sedang ditransfer 
		}
		// try messages that could be delivered to final recipient
		// coba pesan yang dapat dikirimkan ke penerima akhir
	 ...
	 * Returns the message delivery cost between two hosts from this host's
	   ...
	   Mengembalikan cost pengiriman pesan antara dua host dari sudut pandang host ini. 
	   Jika tidak ada jalur antara host "dari" dan "ke", 
	   Double.MAX_VALUE dikembalikan. Jalur hanya dihitung untuk host 
	   yang dituju host ini.
	 * @param from The host where a message is coming from
	   @param dari host tempat pesan itu berasal
	 * @param to The host where a message would be destined to
	   @param ke host tempat pesan ditakdirkan untuk
	 * @return The cost of the cheapest path to the destination or
	   Double.MAX_VALUE if such a path doesn't exist
	   @return Host jalur terpendek ke tujuan atau
	   Double.MAX_VALUE jika jalan seperti itu tidak ada
	 */
	public double getCost(DTNHost from, DTNHost to) {
		/* check if the cached values are OK */
		// cek jika nilai pada cached OK
		if (this.costsForMessages == null || lastCostFrom != from) {
			/* cached costs are invalid -> calculate new costs */
			// cost cached tidak valid > hitung cost baru
			this.allProbs.put(getHost().getAddress(), this.probs);
			int fromIndex = from.getAddress();

			/* calculate paths only to nodes we have messages to (optimization) */
			// hitung jalur hanya ke node yang kita miliki pesannya (pengoptimalan)
			...
			this.lastCostFrom = from; // store source host for caching checks
									// menyimpan sumber host untuk pemeriksaan cache 
		}
		...
		if (costsForMessages.containsKey(to.getAddress())) {
			...
			// tidak ada jalur yang diketahui ke host yang diberikan
	 ...
	 * Mencoba mengirim semua pesan lain ke semua host terhubung yang diurutkan 
	 * berdasarkan jumlah hop dan probabilitas pengirimannya 
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		ArrayList<Message> sendlists = new ArrayList<Message>();

		/* for all connected hosts that are not transferring at the moment,
		 * collect all the messages that could be sent */
		for (Connection con : getConnections()) {
			DTNHost other = con.getOtherNode(getHost());
			MaxPropRouterManajemenBuffer othRouter = (MaxPropRouterManajemenBuffer)other.getRouter();
			...
			for (Message m : msgCollection) {
				...
				//pengecekan double pesan, biar pesan tidak ada yg sama
				int exist = 0;
				for(int z=0; z<sendlists.size(); z++){
					if(sendlists.get(z).getId().equals(m.getId())){
						exist = 1;
					}
				}
		...
		Collections.sort(sendlists, new DataChainedComparator(
		...
		for (Connection con : getConnections()) {
			for (Message msg : sendlists) {
				if(temp_id.contains(msg.getId())){
					//do nothing
				}else{
					messages.add(new Tuple<Message, Connection>(msg,con));
					temp_id.add(msg.getId());
				}
			}
		}
		...
		if(messages.size() != 0){
			System.out.print(String.format("%.2f", SimClock.getTime())+" | ");
			System.out.print("Node: "+super.getHost()+" | Sorted List = ");
			System.out.print("size: "+messages.size()+"; ");
			for(int i = 0; i < messages.size(); i++) {
				Message m = (Message) messages.get(i).getKey();
		        System.out.print(m.getId()+"("+m.getHopCount()+","+m.getTtl()+");");
		    }
			System.out.println("");
		}
	 ...
	 */
	/**
	 * Menghitung dan mengembalikan nilai ambang saat ini untuk pembagian buffer 
	 * berdasarkan pada jumlah rata-rata byte yang ditransfer per peluang transfer 
	 * dan jumlah hop dari pesan dalam buffer. Metodenya bersifat publik hanya 
	 * untuk membuat pengujian lebih mudah.
	 * @return nilai threshold saat ini (hop count) untuk pembagian buffer 
	 */
	public int calcThreshold() {
		/* b, x and p refer to respective variables in the paper's equations */
		// b, x dan p lihat masing-masing variabel dalam persamaan kertas
		...
		if (x == 0) {
			...
			// tidak dapat menghitung threshold karena tidak ada data transfer
			return 0;
		}

		/* calculates the portion (bytes) of the buffer selected for priority */
		// menghitung porsi (byte) dari buffer yang dipilih untuk prioritas
		if (x < b/2) {
			p = x;
		}
		else if (b/2 <= x && x < b) {
		...
		// membuat salinan daftar pesan, diurutkan berdasarkan jumlah hop
		ArrayList<Message> msgs = new ArrayList<Message>();
		msgs.addAll(getMessageCollection());
		if (msgs.size() == 0) {
			return 0; // no messages -> no need for threshold
			          // tidak ada pesan -> tidak memelurkan threshold
		}
		/* anonymous comparator class for hop count comparison */
		// kelas komparator anonim untuk perbandingan jumlah hop
		...
		// menemukan pesan pertama yang berada di luar bagian yang dihitung
		int i=0;
		...
		i--; // the last round moved i one index too far
		     // round terakhir i satu indeks terlalu jauh
		if (i < 0) {
			return 0;
		}
		...
		/* sekarang i menunjuk ke paket pertama yang melebihi porsi p
		 * threshold adalah hop-count paket + 1 (sehingga paket dan
		 * mungkin beberapa lagi termasuk dalam bagian prioritas)
		 */
	 ...
	 */
	/**
	 * Pembanding pesan untuk modul perutean MaxProp
	 * Pesan yang memiliki jumlah hop lebih kecil dari yang diberikan 
	 * threshold diberi prioritas dan mereka diperintahkan oleh jumlah hop mereka.
	 * Pesan lain diperintahkan oleh delivery cost mereka.
	 */
	private class MaxPropComparator implements Comparator<Message> {
		 ...
		 * {@link MaxPropRouterManajemenBuffer#getCost(DTNHost, DTNHost)}) is first.
		 */
	...
	public MessageRouter replicate() {
		MaxPropRouterManajemenBuffer r = new MaxPropRouterManajemenBuffer(this);
		return r;
	}
}
