...
//		}
		...
		if (!makeRoomForIncomingMessage(m.getSize(), m.getHopCount(), m.getTtl())) {
			return DENIED_NO_SPACE; // couldn't fit into buffer -> reject
		}
...
//	Make Room For Incoming Message
	protected boolean makeRoomForIncomingMessage(int size, int hopCount, int ttl){
		if (size > this.getBufferSize()) {
			return false; // message too big for the buffer
		}
...
//		if(size > freeBuffer){
//			System.out.println(String.format("%.2f", SimClock.getTime())+" Node : "+this.getHost()+" not enough space");
//		}
		...
		while (freeBuffer < size) {
			...
			if(hopCount > m.getHopCount()){
				return false;
			} else if(hopCount == m.getHopCount()){
				if(ttl < m.getTtl()){
					return false;
				} else {
					/* delete message from the buffer as "drop" */
